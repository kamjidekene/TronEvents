...
contract EventFactory {
    ...
    event TransferTicketOwnership(
        uint256 eventId,
        address indexed participantAddress,
        uint48 eventTag
    );

    function createEvent(uint48 maxNumberOfParticipants, uint256 _amount)
        public
    {
    ...
    function checkInEventAttendees(
        uint256 _eventId,
        address _address,
        uint32 _eventTag
    ) public {
    ...
    function fetchEventParticipants(uint256 _eventId, uint256 _participants)
        public
        view
        returns (EventTracker[] memory addresses)
    {
